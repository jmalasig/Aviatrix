...
func fly(myPlane : Aviatrix) {
    ...
    for (index, city) in destinations.enumerated() {
        let distance = myPlane.distanceTo(start: myPlane.location, target: city)
...
func refuel(myPlane : Aviatrix) {
    ...
    print("⛽ You refueled \(refuelData) gallons totaling \(plane.fuelLevel)")
}

func fuelCheck(myPlane : Aviatrix, destination : String) -> Bool {
    let distanceToTravel =  Double(myPlane.distanceTo(start: myPlane.location, target : destination))
...
var plane = Aviatrix(authorName: "Joan")
